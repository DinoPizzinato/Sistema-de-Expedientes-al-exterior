(function () {
  ...
  function buildExtradicionTitle(funcionarios) {
    const paises = buildDestinationCountriesText(funcionarios);
    return paises ? `EXTRADICIÓN A ${paises}` : "EXTRADICIÓN";
  }
  ...
  function buildFuncionariosExtradicionText(funcionarios) {
    return `${joinNatural(
      funcionarios.map((funcionario) =>
  ...
  function buildExtradicionPertenenciaClause(funcionario) {
    const dependencia = normalizeSpaces(funcionario.destino);
    if (!dependencia) return "";
    return `perteneciente a ${buildDependenciaConArticulo(dependencia)}`;
  }

  function buildFuncionariosExtradicionText(funcionarios, dependenciaPrincipal) {
      ...
      funcionarios.map((funcionario, indice) => {
        const base =
          indice === 0
            ? formatFuncionarioBase(funcionario)
            : `al ${formatFuncionarioBase(funcionario)}`;
        const dependencia = buildExtradicionPertenenciaClause(
          funcionario,
          dependenciaPrincipal,
        );
  ...
  function buildContext(data) {
    ...
    const nombresConDependencia = buildFuncionariosExtradicionText(
      funcionarios,
      dependenciaPrincipal,
    );
    const extradicionAsuntoTitulo = buildExtradicionTitle(funcionarios);
    ...
    return {
      ...
      expulsion_asunto_titulo: extradicionAsuntoTitulo,
      ...
      elevacion_requerimiento_texto: `Se tomó conocimiento del presente expediente relacionado con el requerimiento efectuado por ${requerimientoBase}, ${authorityRequesterVerb} cumplimentar la extradición de nuestro país del ciudadano de nacionalidad ${nacionalidad} "${nombreDetenido}".`,
      ...
      if_finalizacion_requerimiento_texto: `El presente expediente se encuentra relacionado con el requerimiento efectuado por ${requerimientoBase}, ${authorityRequesterVerb} cumplimentar la extradición de nuestro país del ciudadano de nacionalidad ${nacionalidad} "${nombreDetenido}".`,
      informe_requerimiento_texto: `Se diligencia el presente expediente, acorde a lo dispuesto por la Gestión ADMINISTRATIVA - JEFATURA en ORDEN #14, relacionado con el requerimiento efectuado por ${requerimientoBase}, ${authorityRequesterVerb} cumplimentar la extradición de nuestro país del ciudadano de nacionalidad ${nacionalidad} mencionado anteriormente.`,
      ...
      nota_intro_texto: `En razón del expediente electrónico: ${normalizeSpaces(
        datosJudiciales.expedienteElectronico,
      )}, relacionado con la extradición de nuestro país del ciudadano de nacionalidad ${nacionalidad} "${nombreDetenido}" y para la cual ${
  ...
  function generarDocumentos(datos) {
    ...
    templateOrder.forEach((key, index) => {
      ...
      const outputName =
        key === "expulsion" ? "Extradicion.docx" : definition.outputName;
      generated.push(outputName);
      downloadBlob(outputName, blob, index * 250);
    });

    return generated;
  }

  window.ExtradicionesWord = {
    generarDocumentos,
  };
})();
